...
function monthKey(value) {
  return isoDate(monthStart(value)).slice(0, 7);
}
...
/**
 * Projects every loan month-by-month. A balance adjustment is an absolute balance
 * snapshot, not an extra payment. This makes historical loan transfers/refinances
 * possible without corrupting the normal amortization calculation.
 */
export function projectLoans({ loans = [], updates = [], oneTimePayments = [], balanceAdjustments = [], startDate, years = 50, untilDate = null }) {
  if (!loans.length) return [];

  const loanStartDates = loans.map((loan) => parseDate(loan.start_date)).filter(Boolean);
  const earliest = startDate ? monthStart(startDate) : monthStart(new Date(Math.min(...loanStartDates.map((d) => d.getTime()))));
  ...
  for (let month = 0; month < maxMonths; month += 1) {
    ...
    for (const state of states) {
      ...
      const principal = Math.min(state.amortization, balanceBefore);
